...
const aes = createCipheriv('aes-256-ecb', aesKey, '')

export function decryptData(data, sequence) {
  transformAES(data, sequence)
  decryptMapleCrypto(data)
}
...
export function generateHeader(sequence, length, version) {
  let iiv = (sequence[3]) & 0xFF;
  iiv |= (sequence[2] << 8) & 0xFF00;

  iiv ^= version
  let mlength = ((length << 8) & 0xFF00) | (length >>> 8);
...
export function encryptData(data, sequence) {
  encryptMapleCrypto(data);
  transformAES(data, sequence);
}

function encryptMapleCrypto(data) {
  var length = data.length, j;
  var a, c;
  for (var i = 0; i < 3; i++)
  {
